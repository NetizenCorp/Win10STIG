...
$script:isCorrectChecklist=$null #Holds Boolean Determination if script is using the correct version of the STIG Checklist 
...
$script:isRunAsAdmin=$null #Holds Boolean Determination if Sscript is RunAs Admin
$script:functionsInScript=$null #Holds all function names from this script
$script:functionCount=0 #Holds running total of completed checks
$script:totalNumFunctions=0 #Holds total number of V_ functions in script
$script:isVirtualMachine=$null #Holds Boolean Determination if OS is Virtual
$script:Location=$null #Holds current location of Script Folder Structure
...
function checklistVersionIsOK{
$release=$stigText.CHECKLIST.STIGS.iSTIG.STIG_INFO | 
       Where-Object {$_.SI_DATA.SID_DATA -like "Release: 14 Benchmark Date: 27 Jul 2018"}
$os=$stigText.CHECKLIST.STIGS.iSTIG.STIG_INFO | 
       Where-Object {$_.SI_DATA.SID_DATA -like "Windows 10 Security Technical Implementation Guide"}
 if((($release | Measure-Object).Count -gt 0) -and (($os | Measure-Object).Count -gt 0) ){
  $script:isCorrectChecklist="Y"
 } else {
  $script:isCorrectChecklist="N"
 }  
}
...
function getStigText{  
  ...
  $script:stigText=( Select-Xml -Path .\Seed_ckl\*.ckl -XPath / ).Node   
}
...
{  
   $script:isRunAsAdmin=([Security.Principal.WindowsPrincipal] `
...
&updateStigCommentsField "V-63373" ("Default permissions gathered using the alternative method discussed in the Check Content section. `n`n" `
 + (Get-Content .\Temp\V-63373.txt | Out-String))
Remove-Item .\Temp\V-63373.txt -ErrorAction SilentlyContinue 
}
...
$status="NotAFinding"
try{
 Write-Output "Searching for .p12 and .pfx files.  Do not close this window."
 $drives=Get-WmiObject -Class Win32_logicaldisk -filter "drivetype = 3" -ErrorAction Stop
 foreach($drive in $drives){ 
  $path=$drive.DeviceID + "\"
  if((($results=Get-ChildItem -Path $path -Filter *.p12 -Recurse -File -Name -ErrorAction Stop) `
     | Measure-Object).Count -gt 0){
   $status="Open"
   foreach($result in $results){
    Write-Output ($path + $result) >> .\Temp\V-63393.txt
   }
  }  
  if((($results=Get-ChildItem -Path $path -Filter *.pfx -Recurse -File -Name -ErrorAction Stop) `
     | Measure-Object).Count -gt 0){
   $status="Open"
   foreach($result in $results){
    Write-Output ($path + $result) >> .\Temp\V-63393.txt
...
&updateVulnStatus "V-63393" $status
if($status -eq "Open"){
 &updateStigCommentsField "V-63393" (".p12 or .pfx files found here:`n" + (Get-Content .\Temp\V-63393.txt | Out-String))
}
Remove-Item .\Temp\V-63393.txt -ErrorAction SilentlyContinue
}
...
try{
 if($OSReleaseID -ge 1709 -and $isClassified -eq "N"){
   $location="HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender Security Center\App and Browser protection\"
   $key="DisallowExploitProtectionOverride" 
    $items = get-itemproperty $location -ErrorAction SilentlyContinue | select $key
    if ( $items.$key.count -eq 1 -and $items.$key -eq 1) {
      $status = "NotAFinding"    
    } else {
...
&updateVulnStatus "V-77025" $status
}
...
if($isRunAsAdmin){
 cd..
 &getStigText 
 if(($stigText | Measure-Object).Count -gt 0){ 
  &checklistVersionIsOK
  if($isCorrectChecklist -eq "Y"){
   &getcomputerName;&getOSReleaseID 
   ...
   $script:totalNumFunctions=($functionList | Measure-Object).Count
   foreach($function in $functionList){
   $script:functionCount=$script:functionCount + 1
   &$function #Calling all "V_" functions in script
   Write-Output "($functionCount/$totalNumFunctions) Check $function Complete"  
   &saveUpdatedCkl #Write completed checklist   
  }
  cls
  Write-Output "Script executed successfully."
  $reportLocation="The updated CKL is located here: " + (Get-Location) + "\Reports"
  Write-Output $reportLocation 
  } else {Write-Output "This Script Requires Checklist Version:`nWindows 10 Security Technical Implementation Guide :: Release: 14 Benchmark Date: 27 Jul 2018"} 
 } else { Write-Output "*****No Checklist Found in the Seed_ckl Folder*****"}  
} else {
